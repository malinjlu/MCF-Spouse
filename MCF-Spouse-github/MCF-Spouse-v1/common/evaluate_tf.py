...
def avgprec(outputs, test_target):
    ...
    for i in range(test_data_num):
        if np.sum(test_target[i]) != class_num and np.sum(test_target[i]) != 0:
            instance_num = instance_num + 1
            temp_outputs.append(outputs[i])
            temp_test_target.append(test_target[i])
            labels_size.append(np.sum(test_target[i] == 1))
    ...
    for i in range(instance_num):
        ...
        for j in range(labels_size[i]):
            loc = findIndex(labels_index[i][j], index)
            summary = summary + np.sum(indicator[loc:class_num]) / (class_num - loc)
...
def Performance(predict_labels, test_target):
    ...
    tempTar[tempTar == 0] = -1
    com = np.sum(tempPre == tempTar)
    tempTar[tempTar == -1] = 0
    PreLab = np.sum(tempPre)
    TarLab = np.sum(tempTar)
...
def Fmacro(predict_labels, test_target):
    Fma = metrics.f1_score(test_target, predict_labels, average='macro')
    return Fma

def Fmicro(predict_labels, test_target):
    Fmi = metrics.f1_score(test_target, predict_labels, average='micro')
    return Fmi
